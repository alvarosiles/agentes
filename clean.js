...
function limpiarTexto(texto) {
	...
	resultado = resultado.replace(/<SView\b[^>]*>/g,
		(match) => {
			return match.replace(/\s*\/\/.*$/gm, "").replace(/\s*\n\s*/g, " ").replace(/\s{2,}/g, " ").replace(/\s+=\s+/g, "=").trim();
		}
	);
	...
	resultado = resultado.replace(
		/style=\{\{([\s\S]*?)\}\}/g,
		(match, contenido) => {
			return `style={{ ${contenido.replace(/\s*\/\/.*$/gm, "").replace(/\s*\n\s*/g, " ").replace(/\s{2,}/g, " ").trim()} }}`;
		}
	);

	resultado = resultado.replace(
		/(\bconst\b|\blet\b|\bvar\b)?\s*(\w+)\s*=\s*(await\s+)?([a-zA-Z0-9_.]+)\(([\s\S]*?)\)/g,
		(match, decl, name, aw, fn, args) => {
			if (match.includes("=>") || match.includes("function")) return match;
			let limpio = args.replace(/\s*\/\/.*$/gm, "").replace(/\s*\n\s*/g, " ").replace(/\s{2,}/g, " ").trim();
			return `${decl ? decl + " " : ""}${name} = ${aw || ""}${fn}(${limpio})`;
		}
	);
